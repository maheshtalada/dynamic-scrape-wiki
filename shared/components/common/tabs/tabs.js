...
export default class Tabs extends Component {
	...
	renderTabs() {
		...
		return (
			tabs.map(tab => {
				return (
					<Tab tooltipConfig={this.props.tooltipConfig} activeSubTab={activeSubTab} active={activeTab === tab.key} tab={tab} context={pageContext} onTabSelect={this.onTabSelected}/>
				)
			})
		)
	}
	...
	render() {
		...
		return (
			<div className={Cx("flex",screenSize > mobileTabsScreen ? "tabs-wrap flex-align-center" : "mobile-tabs-wrap")}>
				<div className={Cx("tabs",className,slider ? 'with-slider' : 'no-slider')}>
					{slider ? <TabSlider settings={{"className" : "tabs__slider"}}>
						{this.renderTabs()}
