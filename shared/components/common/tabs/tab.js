...
export default class Tab extends Component {
	...
	render() {
		...
		return (
			<div ref={active ? "activeTab": "tab"} onMouseEnter={this.onShowOptions} onMouseLeave={this.onHideOptions} className={Cx("tabs__tab-wrap hvr-rectangle-out",{'active': active})}>
				<button className="tabs__tab-btn hvr-grow" onClick={this.onTabSelected} data-for={`tab-tooltip-${label}`}  data-tag-label={l(label)} data-tag-action="Tab Click" data-tag-category={`Tabs - ${this.props.context}`}>
				...
				{tab.options && <div className={Cx("tabs__options",{'show': showOptions})}>
					<ul>
						{tab.options.map((option,index) => {
							return (
								<li className={Cx({'active': activeSubTab === option.value.key})}>
									<button className="tabs__options__option-btn" data-tag-label={l(option.label)} data-tag-action="Tab Click" data-tag-category={`Tabs - ${this.props.context}`} onClick={()=>{this.onOptionSelected(option,index)}}>
										{l(option.label)}
										{option.tooltip && <i className="tooltip pe-7s-help1" data-for={`tab-tooltip-${label}`} data-tip={l(option.tooltip)}/>}
									</button>
								</li>
							)
						})}
				...
				<ReactTooltip place="bottom" id={`tab-tooltip-${label}`}/>
			</div>
		)
	}
}
