...
export default class MapFeature extends Component {
	...
	async onMapLoad(map, maps){
		...
		if(!frameworkGlobals.isServer) {
			try {
				const responseObject = await fetch(`${window.__CONFIG__.assetsPath}/metro-geo-json${deviceConfig.filePath}?v=2`);
	...
	render() {
		...
		return (
			<div id="median-map-feature" className={Cx("median-map-feature",key)}>
				...
				{mapRangeLegends && <MapLegends mapRangeLegends={mapRangeLegends} legendBoxConfig={legendBoxConfig} tooltipMsg={tooltipMsg}/>}
	...
	styleFeature(feature) {
		...
		return {
			...
			strokeColor: "#fff",
			zIndex: zIndex,
			fillColor: feature.getProperty('type') && legendBoxConfig.colorCodes[feature.getProperty('type')] || 'transparent',
			fillOpacity: 0.6
		};
	}
