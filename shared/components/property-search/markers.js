...
import {getHintBottomOffsetClass, getHintBaloonVerticalPosClass, getHintBaloonHorizontalPosStyle} from '../../utils/balloon_pos.js';
import {getMarkerHolderStyle, getMarkerStyle } from '../../utils/marker_styles.js';
...
export default class Marker extends Component {
	static propTypes = {
		...
		showBallonState: PropTypes.bool,
		onShowBallonStateChange: PropTypes.func,
	...
	render() {
		...
		const { key, style, lat, lng, price } = this.props;
		...
		/* <div key={key} style={style} lat={lat} lng={lng}>{price}</div>*/
		return (

			<div
			 	style={markerHolderStyle}
			 	className={cx('map-marker hint hint--html',this.props.hintType,hintBalloonBottomOffsetClass,noTransClass, noTransBalloonClass, hintBaloonVerticalPosClass,
			 	this.props.showBallon ? 'hint--balloon' : '',showHint ? 'hint--always' : 'hint--hidden')}>
				 <div style={styleMarkerMarker} className={cx('map-marker__marker', imageClass)}>
				 	 <div className="map-marker__marker__text" >{price}</div>
				</div>
				 <div
					 style={{'display':'none'}}
					 ...
					 onMouseLeave={this._onMouseLeaveContent}>
						 ...
						 <div>
						 <a className={cx('map-marker-hint__ap-link', this.props.showBallon ? 'map-marker-hint__ap-link--hidden' : '')}>{`${l('VIEWMOREINFO')}`}</a>
						 </div>
				 </div>
			</div>
		);
