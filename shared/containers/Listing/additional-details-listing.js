...
import { REQUEST_ADD_ADDITIONAL_SCHEMA } from '../../redux/actions/schema';

const NEXT = 'next',
	  FINISH = 'finish';
class AdditionalDetailsListing extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			modifiedValues: {},
			initialValues: {},
			isFetching: (props.schema_get_additional && props.schema_get_additional.isFetching) || false,
	...
	componentWillReceiveProps(props) {
		const { schema_get_additional } = props;
		this.setState({
			isFetching: schema_get_additional && schema_get_additional.isFetching
		});
		if(props.schema_save_additional && props.schema_save_additional.listingid) {
			this.handlePostOnline(props.schema_save_additional);
			return;
		} else if(props.schema_save_additional && props.schema_save_additional.isFetching) {
			this.setState({
				isFetching : props.schema_save_additional.isFetching
			});
			return;
		}
		if(props.schema_save_additional && (props.schema_save_additional.error || props.schema_save_additional.status === 'error')) {
			window.scrollTo(0,0);
			this.setState({
				isError: true,
				errorCode: props.schema_save_additional.error && props.schema_save_additional.error.error_description
			});
		}
	}
	...
	handlePostOnline(props) {
		...
		if(goto && goto === FINISH ) {
			...
			this.context.router.push({
				pathname : `/profile/media/property-listing/${props.listingid}`
			});
		}
	...
	onSchemaChange(changeObject, hasErrors) {
		const schema_get_additional = this.props;
		this.setState({
			changeObject,
			hasErrors
		});
		let schemaLength = values(schema_get_additional.schemas).length;
	...
	renderSchema() {
		...
		const { schema_get_additional } = this.props;
		if (isEmpty(schema_get_additional)) {
			return null;
		}
		let expressList = schema_get_additional.schemas,
			schemaInfo = schema_get_additional.schemaInfos,
			referenceData = schema_get_additional.referenceData,
	...
	onSubmit(goto) {
		let schemasAreValid = true,
			schemaLength = values(this.props.schema_get_additional.schemas).length,
			schemaGroups = [];
		...
		if (schemasAreValid) {
			...
			this.props.dispatch(REQUEST_ADD_ADDITIONAL_SCHEMA({
	...
	onBackClick() {
		this.context.router.push({
			pathname : `/profile/financial/property-listing/${this.props.params.id}`
		});
	}
...
const mapStateToProps = ({schema}) => {
	const { schema_save_additional } = schema;
	return {
		'schema_save_additional': schema_save_additional
	};
};

export default connect(mapStateToProps)(AdditionalDetailsListing);
